...
template <typename Float, typename Spectrum>
class ParticleAccumulatorDebugIntegrator final : public VolumeIntegrator<Float, Spectrum> {
public:
    ...
    ParticleAccumulatorDebugIntegrator(const Properties &props) : Base(props) { 
    ...
    trace_light_ray(Ray3f ray, const Scene *scene, Sensor *sensor,
        ...
        while (loop(active)) {
            ...
            /* ----------------------- BSDF sampling ------------------------ */
                ...
                Mask absorption_event = sampler->next_1d(active) >= unpolarized_spectrum(bsdf_val)[0] * correction;
                if ( dr::any_or<true>(absorption_event && active)) {
                    // Apply filtering
                    Mask pass = active;
                    ...
                    sensor->accumulate(
                        ...
                        /*filter=*/pass, 
                        /*active=*/absorption_event && active);
            ...
            active &= depth < (uint32_t) m_max_depth;
            Log(Debug, "throughput: %d, active: %d", throughput, active);
        }
    ...
    std::string to_string() const override {
        return tfm::format("ParticleAccumulatorDebugIntegrator[\n"
                           "  min_depth = %u,\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "  periodic_box = %d\n"
...
MI_IMPLEMENT_CLASS_VARIANT(ParticleAccumulatorDebugIntegrator, VolumeIntegrator);
MI_EXPORT_PLUGIN(ParticleAccumulatorDebugIntegrator, "Particle Tracer integrator");
