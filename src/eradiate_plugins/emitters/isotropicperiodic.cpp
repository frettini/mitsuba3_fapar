...
.. _emitter-isotropicperiodic:

Isotropic periodic emitter (:monosp:`isotropicperiodic`)
---------------------------------------------------
...
This emitter plugin implements a isotropic source which radiates a
...
.. tabs::
    .. code-tab:: xml
        :name: isotropicperiodic-light

        <emitter type="isotropicperiodic">
...
MI_VARIANT class IsotropicPeriodicEmitter final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_to_world, m_needs_sample_3)
    MI_IMPORT_TYPES(Scene, Texture, Shape)

    IsotropicPeriodicEmitter(const Properties &props) : Base(props) {

        if( props.has_property("to_world"))
            Throw("Found a 'to_world' transformation -- this is not allowed. "
                  "The isotropic peridoic emitter only hemits in the hemispherice "
                  "described by normal [0., 0., -1.].");
        

        m_to_world = ScalarTransform4f::look_at(
            0.f, ScalarPoint3f(0.f,0.f,-1.f), ScalarVector3f(0.f,1.f,0.f));
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        // 1. "Sample" isotropic component (fixed, no actual sampling required)
        const auto trafo = m_to_world.value();
        Vector3f local = warp::square_to_cosine_hemisphere(direction_sample);
        Vector3f d_global = trafo.transform_affine(local);
        ...
        Spectrum weight = wav_weight * extent.x() * extent.y() * dr::Pi<Float>;
    ...
    std::string to_string() const override {
        ...
        oss << "IsotropicPeriodicEmitter[" << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(IsotropicPeriodicEmitter, Emitter)
MI_EXPORT_PLUGIN(IsotropicPeriodicEmitter, "Isotropic periodic emitter")
